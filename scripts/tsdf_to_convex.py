...
ms.add_mesh(pymeshlab.Mesh(verts, faces), "o3d_mesh")

# print("methods of a mesh: ", dir(ms.current_mesh()))
print(f"vertex count before small component removal: {ms.current_mesh().vertex_number()}")
print(f"  face count before small component removal: {ms.current_mesh().face_number()}")
# 細かい連結面をremove connected components by diameterで消す
ms.apply_filter('meshing_remove_connected_component_by_diameter',
                mincomponentdiag=pymeshlab.PureValue(0.030))  # 30mm以下の独立したパーツを消去します
# 10面以下の小さなパーツも消す
ms.apply_filter('meshing_remove_connected_component_by_face_number',
                mincomponentsize=10)  # 10面以下の独立したパーツを消去します
# さらに、細かいエッジや頂点を削除してからリメッシュすることで、Uniform Mesh Resamplingの品質が向上します
ms.apply_filter('meshing_isotropic_explicit_remeshing', 
                targetlen=pymeshlab.PureValue(0.010), # Voxelサイズより少し大きい程度
                iterations=3, 
                adaptive=True)
print(f"vertex count before resampling: {ms.current_mesh().vertex_number()}")
print(f"  face count before resampling: {ms.current_mesh().face_number()}")
# Uniform Mesh Resamplingフィルタを適用
print("Applying Uniform Mesh Resampling...")
start_time = time.time()
ms.apply_filter("generate_resampled_uniform_mesh",
                cellsize=pymeshlab.PureValue(0.0025), # cellsize
                offset=pymeshlab.PureValue(0.01),     # offset
                mergeclosevert=True,                      # mergeclosevert
                discretize=False, 
                multisample=False, 
                absdist=True)                             # absdist
end_time = time.time()
print(f"vertex count after resampling: {ms.current_mesh().vertex_number()}")
print(f"  face count after resampling: {ms.current_mesh().face_number()}")
print(f"Uniform Mesh Resampling took {end_time - start_time:.2f} seconds\n")
print("Applying HC-Laplacian Smoothing...")
start_time = time.time()
ms.apply_filter("apply_coord_hc_laplacian_smoothing")
end_time = time.time()
print(f"vertex count after smoothing: {ms.current_mesh().vertex_number()}")
print(f"  face count after smoothing: {ms.current_mesh().face_number()}")
print(f"HC-Laplacian Smoothing took {end_time - start_time:.2f} seconds\n")
# visualize_pymeshlab(ms.current_mesh())
# o3d_watertight_check(ms.current_mesh())

print("Applying Quadric Edge Collapse Decimation...")
start_time = time.time()
ms.apply_filter("meshing_decimation_quadric_edge_collapse",
                ...
                selected=False)
end_time = time.time()
print(f"vertex count after simplification: {ms.current_mesh().vertex_number()}")
print(f"  face count after simplification: {ms.current_mesh().face_number()}")
print(f"edge number after simplification: {ms.current_mesh().edge_number()}")
o3d_watertight_check(ms.current_mesh())
print(f"Quadric Edge Collapse Decimation took {end_time - start_time:.2f} seconds\n")
# decimateしたmeshをexportする
