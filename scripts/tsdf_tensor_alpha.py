...
class TSDFIntegrator(Node):
    ...
    def create_safe_thick_pcd(self, thickness=0.008, repeat=6):
        ...
        combined_points = vertices.clone()  # 最終的に表面と裏面を統合するためのTensor
        for cnt in range(repeat):
            # 2. 裏面候補点の計算 (P' = P - n * d)
            offset_points = vertices - normals * (thickness * (cnt + 1))  # [N, 3] Tensor
            ...
            combined_points = o3c.concatenate([combined_points, safe_offset_points], axis=0)
...
def main(args=None):
    ...
    try:
        ...
        node.save_mesh()
        pcd = node.create_safe_thick_pcd(thickness=0.005, repeat=10)
        if pcd is not None:
            o3d.io.write_point_cloud("safe_thick_pcd.ply", pcd.to_legacy())
            node.alpha_shape(pcd, alpha=0.03)
    finally:
        node.destroy_node()
        # rclpy.shutdown()
